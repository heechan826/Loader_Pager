...
{	
	...
	printf("[Load 1st Loadable segment]");
	...
	for (i = 0; i < ep.e_phnum; i++) 
	{
		...
		printf("phdr.p_vaddr: %p \t phdr.p_offset: %d \t phdr.p_vaddr + phdr.p_memsz: %p \n", phdr.p_vaddr, phdr.p_offset, phdr.p_vaddr + phdr.p_memsz);
...
void segv_handler(int signo, siginfo_t *siginfo ,void* context)
{
	...
	for (i = 0; i < ep.e_phnum; i++) 
	{
		...
		if ((phdr.p_vaddr <= siaddr) && siaddr <= (phdr.p_vaddr + phdr.p_memsz)) 
		{
			...
			switch(heuristic){
				...
				break;  
			    case 'O' : 
				if(3*(count-1)+1<OPTIMAL) next_index = optimal[3*(count-1)+1];
				break; 
			    default :    
				break;
			}
	...
	if(next_index>=0) 
	{
		...
		switch(heuristic){
			...
			break;  
		    case 'O' : 
			if(3*(count-1)+2<OPTIMAL) next_index = optimal[3*(count-1)+2];
			break; 
		    default :    
			break;
		}
	...
	// 13. Invoke 3rd page prediction algorithm
	if(next_index>=0) 
	{
		printf("Third  page: ");
		predict_map(next_index);
